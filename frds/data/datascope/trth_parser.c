...
void process(gzFile file, struct Fields *meta, const char *output_dir, int replace);
...
void process(gzFile file, struct Fields *meta, const char *output_dir, int replace)
{
    ...
    // Read line by line.
    while (gzgets(file, buffer, BUFSIZE))
    {
               ...
               ssize_t chunk_size, const char *output_dir, int replace)
{
...
#ifdef _WIN32
    snprintf(output_file, path_len, "%s\\%s.csv.gz", path, local_date);
#else
    snprintf(output_file, path_len, "%s/%s.csv.gz", path, local_date);
#endif
#if DEBUG
    ...
    {
        // File exists.
        outputFile = gzopen(output_file, mode);
        // Write header row if file exists but we're to overwrite it.
        if (replace == 1)
            gzprintf(outputFile, "%s", fields);
    }
    else
    {
    ...
    {
        gzprintf(outputFile, "%s", chunk[i]);
        free(chunk[i]);
        chunk[i] = NULL;
    }
    gzclose(outputFile);
    printf("Saved chunk RIC: %s, local date: %s at %s (%ld rows)\n",
           RIC, local_date, output_file, chunk_size);
    free(output_file);
    return 0;
}
