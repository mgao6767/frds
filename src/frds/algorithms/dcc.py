import numpy as np
from scipy.optimize import minimize


def dcc(data: np.ndarray) -> np.ndarray:
    """Estimate DCC

    Args:
        data (np.ndarray): (2,T) array of firm and market volatility-adjusted returns

    Returns:
        np.ndarray: (2,) array of (a, b) for the DCC model
    """
    _, T = data.shape

    # Sample covariance matrix, (2,2) array
    Q_avg = np.zeros([2, 2])
    for t in range(T):
        Q_avg += np.outer(data[:, t], data[:, t])
    Q_avg /= T

    def calc_R(a, b):
        Qs = [Q_avg]
        omega = (1.0 - a - b) * Q_avg
        for t in range(T):
            et_1 = data[:, t]
            Qt_1 = Qs[-1]  # Q(t-1)
            Qt = omega + a * np.outer(et_1, et_1) + b * Qt_1
            Qs.append(Qt)
        Rs = []
        for q in Qs:
            tmp = 1.0 / np.sqrt(np.abs(q))
            tmp = tmp * np.eye(2)
            R = np.dot(np.dot(tmp, q), tmp)
            Rs.append(R)
        return Rs

    def loss_func(ab: np.ndarray) -> float:
        """Negative loglikelihood as a function of (a,b)

        Args:
            ab (np.ndarray): (2,) array of (a, b)

        Returns:
            float: negative log likelihood given (a,b)
        """
        R = calc_R(ab[0], ab[1])

        def dcc_loss():
            loss = 0.0
            for t in range(T):
                Rt = R[t]
                Rt_ = np.linalg.inv(Rt)
                et = data[:, t]
                loss += np.log(np.linalg.det(Rt)) + np.dot(np.dot(et, Rt_), et)
            return loss

        return dcc_loss()

    # Solve for a, b
    res = minimize(
        loss_func,  # -ve log likelihood as a function of (a,b)
        [0.5, 0.5],  # initial values for a, b
        method="SLSQP",  # allow for constraints below
        constraints=[
            {"type": "ineq", "fun": lambda x: 1.0 - x[0] - x[1]},  # a+b<1
            {"type": "ineq", "fun": lambda x: x[0]},  # a>0
            {"type": "ineq", "fun": lambda x: x[1]},  # b>0
        ],
        options={"disp": False},
    )

    a, b = res.x[:2]  # a, b

    return a, b
